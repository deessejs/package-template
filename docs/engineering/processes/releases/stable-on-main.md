...
## Why `main` is not the canary branch

`main` only receives changes that have already been validated on `staging`.
The release engineer opens the `staging → main` PR explicitly — it is a
...
## Why this job uses `changesets/action@v2` (and canary does not)
...
If we ever needed an "ephemeral stable" mode (e.g. a dry-run), the same raw
...
`cancel-in-progress: false`. A release that is mid-flight when a new commit

...
The trade-off: GitHub Packages is not a substitute for npm. Public
...
A single `npm publish` invocation cannot target two registries at
...
- `publish-gh-packages` uses `GITHUB_TOKEN`. Best-effort mirror;
  failure does not block the npm publish.

The same pattern works if more registries are added later (e.g. a
...
- `permissions.packages: write` is the new permission required for
  the job. The base workflow does not declare this; each job
  declares its own.

## What gets mirrored

Only the publishable package. Today that is
...
and disposed of at the end. It cannot be exfiltrated from the job
...
2. **Re-running** the failed `publish-gh-packages` job retries
   ...
   version if the previous one was deleted; the workflow does not
   delete-on-fail.
...
GitHub Packages does not enforce the "1 version per package tag"
...
1. **Source-of-truth semantics.** Is npm the source of truth and
   ...
   the other.
2. **Retention.** GitHub Packages does not surface a retention

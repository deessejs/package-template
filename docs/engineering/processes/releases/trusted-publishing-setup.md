...
| Repository           | `<owner>/<repo>` (e.g. `acme/widget`)    |
...
3. **`registry-url: 'https://registry.npmjs.org'`** on the `setup-node`
   call. Without it, OIDC cannot resolve.
...
`pnpm setup` renames the publishable package (e.g. `@deessejs/example` →
`@your-scope/your-package`). The workflow file name does not change, but
...
- The job is running on a GitHub-hosted runner. **Self-hosted runners
  cannot mint OIDC tokens for npm Trusted Publishing** — this is enforced
...
contributor adds a step that does (e.g. to install from a private registry),
...
fail because `GITHUB_TOKEN` cannot push to the protected `main` branch
...
- Workflow file is renamed or moved (e.g. `release.yml` → `release-v2.yml`).

...
4. Trusted publisher entry on npmjs.com does not match the running
...
2. The version was published by a different publisher (e.g. a manual
...
commit that was not authored by you, OR `.changeset/*.md` files are
...
package name (e.g. `@deessejs/example` after `pnpm setup` renamed it).

**Likely cause**: `pnpm setup` did not update the trusted publisher
...
after the failed publish, do not push — this creates version drift. The
...
- If the previous run failed **before** `npm publish` succeeded, the
  re-run succeeds (the version does not yet exist on npm).
...
If canaries are causing persistent failures (e.g. npm registry issues
...
Do not disable the stable job unless absolutely necessary — a disabled
stable job means merges to `main` do not produce npm releases.
...
For very serious incidents (e.g. security vulnerability), follow the
...
`@canary` is acceptable; a broken stable on `@latest` is not):
...
The canary-on-`@canary` errors are by design opt-in and do not require

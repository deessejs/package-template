...
What we test, how, and what we explicitly do not test.
...
in `tests/fixtures/corpus/`. They do not touch the filesystem beyond
reading fixtures.

## Integration tests

Each command has one integration test that spawns the CLI binary
(`node bin/vgpu.mjs …`) and asserts on stdout, stderr, and exit code.
...
the **real corpus** at `apps/web/.source`. It is skipped unless
`process.env.VGPU_SMOKE=1` is set, so it does not break local
...
Fixtures live under `tests/fixtures/`. They are committed and
...
├── vgpu/
│   ├── surface.docs.md      # hosts three symbols in one file
│   ├── buffer.docs.md
│   └── malformed.docs.md    # intentionally bad frontmatter
└── vgpu-web/
    └── buffer.docs.md       # same title as vgpu/buffer.docs.md (duplicate)
```
...
- **Colour or terminal rendering.** The CLI does not produce colour.
  Nothing to test.
...
- **npm packaging.** `apps/cli/package.json` uses the same allowlist
  ...
  the package shape breaks, `packages/example/` breaks too; we do not
...
will surface it but the workflow does not block.

## Local loop

```bash
# Fast loop: unit + integration, no smoke
pnpm --filter vgpu test:run

# Full loop: includes smoke against the real corpus
pnpm --filter web build
VGPU_SMOKE=1 pnpm --filter vgpu test:run
```

# `vgpu docs` — Internal Architecture

Architecture documents for the `vgpu docs` command-line interface. Read
...
4. [`04-corpus.md`](./04-corpus.md) — where the corpus comes from, how
   it is indexed, and the option chosen (A / B / C).
5. [`05-testing.md`](./05-testing.md) — what is tested, fixtures, and
   the boundary between unit and end-to-end tests.
6. [`06-roadmap.md`](./06-roadmap.md) — implementation phases, their
   dependencies, and exit criteria.

## Status

This documentation describes a CLI that has not been scaffolded yet. It
is the design contract that implementation must satisfy. Changes to the
CLI that diverge from these documents should land **with** an update to
the relevant document in the same PR.
